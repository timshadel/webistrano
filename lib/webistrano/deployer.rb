  ...
  class Deployer
    ...
    def execute!
      ...
      status = catch(:abort_called_by_capistrano){
        ...
        load_project_custom_recipes(config)
    ...
    def load_project_custom_recipes(config)
      begin
        deployment.stage.project.recipes.each do |recipe|
          logger.info("loading project recipe '#{recipe.name}' ")
